...
doc.transact([&](am::Transaction& tx) {
    ...
    am::import_json(tx, input, am::root);  // same transaction
});
```
...
am::import_json(doc, json{{"timeout", 30}}, *config_id);
```
...
```cpp
auto config_id = doc.get_obj_id(am::root, "config");
auto config_json = am::export_json(doc, *config_id);
...
// ... more changes happen ...
auto snapshot = am::export_json_at(doc, heads);
```
...
am::put_pointer(doc, "/config/database/port", am::ScalarValue{std::int64_t{3306}});

// Create intermediate maps automatically
am::put_pointer(doc, "/new/nested/path", am::ScalarValue{std::string{"hello"}});
...
```cpp
am::delete_pointer(doc, "/config/database/host");
// The "host" key is removed from the database map

am::delete_pointer(doc, "/items/0");
...
auto val = am::get_pointer(doc, "/a~1b");  // 42
```
...
```cpp
auto doc = am::Document{};
am::import_json(doc, json{{"items", {10, 20, 30}}});

// Read by index
auto second = am::get_pointer(doc, "/items/1");  // 20

// Append to list with "-"
am::put_pointer(doc, "/items/-", am::ScalarValue{std::int64_t{40}});
...
auto diff = am::diff_json_patch(doc1, doc2);
// [
...
auto result = am::export_json(doc);
// {
...
```cpp
auto config_id = doc.get_obj_id(am::root, "config");
am::apply_merge_patch(doc, json{{"lang", "fr"}}, *config_id);
```
...
```cpp
auto doc1 = am::Document{};
am::import_json(doc1, json{{"x", 1}, {"y", 2}, {"z", 3}});
...
```cpp
auto config_id = doc.get_obj_id(am::root, "config");
auto config_flat = am::flatten(doc, *config_id);
// {"/theme": "dark"}
```
...
am::import_json(original, json{{"a", {{"b", 1}}}, {"c", 2}});
auto flat = am::flatten(original);

auto restored = am::Document{};
am::unflatten(restored, flat);
assert(am::export_json(original) == am::export_json(restored));
```
...
auto original_json = am::export_json(doc);
auto loaded_json = am::export_json(*loaded);
...
All functions are in `namespace automerge_cpp`. Include `<automerge-cpp/json.hpp>`.
