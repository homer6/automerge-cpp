...
**Minimal external dependencies.** No Taskflow, no TBB, no execution policies. The thread pool
uses Barak Shoshany's BS::thread_pool (header-only, fetched via CMake FetchContent) for
work-stealing parallelism with `parallelize_loop`.
...
// Shared pool across multiple documents
auto pool = std::make_shared<detail::ThreadPool>(16);
...
**Lock-free reads** (`set_read_locking(false)`) are available for read-heavy workloads
where the caller guarantees no concurrent writers. This eliminates shared_mutex contention
(51% of CPU in profiling) and achieves **13.5x parallel read scaling** on 30 cores at 100K
keys (vs 1.9x with locking enabled).
...
Built on `std::shared_mutex` for thread safety and Barak Shoshany's BS::thread_pool
(header-only, fetched via CMake FetchContent) for work-stealing parallelism.
...
properties. The internal thread pool (BS::thread_pool) avoids the overhead of
...
### 11C.1 — Internal thread pool (`include/automerge-cpp/thread_pool.hpp`)
- **File:** `include/automerge-cpp/thread_pool.hpp`
- **What:** Barak Shoshany's BS::thread_pool — a header-only, work-stealing thread pool.
  Provides `parallelize_loop` for partitioning work across threads and `submit` for
  async task submission. Configured with `sleep_duration = 0` (yield instead of 500us sleep)
  for low-latency benchmarks.
- **API used by Document:**
  ```cpp
  using thread_pool = BS::thread_pool;

  // Shared across documents via std::shared_ptr<thread_pool>
  auto pool = std::make_shared<thread_pool>(num_threads);

  // Parallel loop: partition [0, count) across workers
  pool->parallelize_loop(count, [&](std::size_t begin, std::size_t end) {
      for (auto i = begin; i < end; ++i) { /* work */ }
  }).wait();
  ```
- **Design rationale:** Header-only, well-tested, work-stealing for non-uniform workloads.
  Workers yield when idle — zero CPU when unused.
- **Risk:** LOW — widely used library, fetched via CMake FetchContent.
...
- **What:**
  ...
  **Part A — Constructors:**
  ...
  explicit Document(std::shared_ptr<ThreadPool> pool);       // externally-owned pool
  ```
  Document stores `std::shared_ptr<ThreadPool> pool_` (nullptr when `num_threads == 1`).
  `fork()` shares the same pool (shared_ptr copy). The pool can be shared across documents.
...
### 11C.2b — Lock-free read path (`set_read_locking`)
- **Files:** `include/automerge-cpp/document.hpp`, `src/document.cpp`
- **What:** Add `set_read_locking(bool)` and `read_locking()` methods to Document.
  When disabled, read methods skip the `std::shared_lock` entirely using a conditional
  RAII guard (`ReadGuard`). Safe when the caller guarantees no concurrent writers.
- **Discovery:** `perf record` on parallel reads showed **51% of CPU** in
  `pthread_rwlock_lock`/`unlock` and **23% in kernel futex** — only 5% was actual
  map lookup work. The `shared_mutex` reader-count atomic bounces a cache line across
  all 30 cores via MESI protocol. IPC was 0.49 (should be 2-4).
- **Results (30-core Xeon, 1M keys):**
  - Before: 10.2M ops/s parallel (1.9x vs sequential)
  - After: **55.0M ops/s parallel (9.6x vs sequential)**
  - IPC: 0.49 → **1.16**
- **Risk:** LOW — read methods only call const functions on DocState. `std::map::find`
  and `std::unordered_map::find` are safe for concurrent const access per the C++ standard.
  Writer exclusion is the caller's responsibility when read locking is disabled.
...
| 13 | 11C.1 Thread pool (Barak Shoshany's BS::thread_pool) | 1 hr | Foundation for parallelism | **DONE** |
| 14 | 11C.2 Thread-safe Document + constructor(num_threads/pool) | 1.5 hr | N concurrent readers | **DONE** |
| 14b | 11C.2b Lock-free reads (`set_read_locking(false)`) | 30 min | **13.5x parallel get at 100K keys** | **DONE** |
...
| 22 | 11E.1-4 Benchmarks + tests | 1.5 hr | Validation | **PARTIAL** |

---

## Parallelism Map: What Runs on 30 Cores

All parallelism uses the Document's internal BS::thread_pool (work-stealing).
When `num_threads == 1`, all paths are sequential.
...
| File | Description |
|------|-------------|
| `include/automerge-cpp/thread_pool.hpp` | Barak Shoshany's BS::thread_pool (header-only, work-stealing) |
